...
Oracle Forge uses three context layers loaded before every session. Each layer is a distinct
mechanism — not a single combined prompt.

| Layer | Source | Mechanism | When updated |
|---|---|---|---|
| **Layer 1 — Schema & Tools** | `agent/AGENT.md` (this file) | Loaded verbatim at session start by `conductor.py` | Manually, when MCP tools or join keys change |
| **Layer 2 — Domain Knowledge** | `kb/domain/dab_*.md` | Dataset-matched files injected by `conductor.py` based on query routing | Manually by Intelligence Officers; auto-updated by `utils/schema_introspector.py` |
| **Layer 3 — Corrections Log** | `kb/corrections/corrections_log.md` | Appended to context before each trial; new entries written by `utils/autodream.py` after failures | Auto-updated by autodream after each benchmark run |

**How each layer is populated:**
- Layer 1 is written and maintained by Drivers. It captures all MCP tool names, join key transformations, and dialect rules needed for any query.
- Layer 2 files are generated by `utils/schema_introspector.py` (pulls live MCP schemas + DAB hints), then enriched by Intelligence Officers with domain terms, query patterns, and injection tests.
- Layer 3 is populated automatically: after each benchmark run, `utils/autodream.py` reads the trial trace, identifies failures, classifies them by type, and appends structured corrections to `corrections_log.md`.

**How each layer is read:**
- `conductor.py` loads AGENT.md at plan_node startup.
- `conductor.py` loads the relevant `dab_*.md` files for each dataset in the query at plan_node startup.
- `conductor.py` loads `corrections_log.md` at plan_node startup, filtered to the relevant dataset.

...
```mermaid
graph TB
    subgraph Governance ["Governance: AWS AI-DLC Framework"]
        A[Inception: Define Done & FAQs] --> B[Construction: Mob Co-Piloting]
        B --> C[Operations: Persistent Artifacts]
    end

    subgraph Context ["Multi-Layer Context Architecture"]
        D1[L1: Schema & Metadata]
        D2[L2: Institutional/Domain Knowledge]
        D3[L3: Interaction Memory/Corrections]
        
        D2 --- KB[Karpathy Knowledge Base]
        D3 --- CL[Self-Learning Corrections Log]
    end

    subgraph Execution ["Self-Correcting Execution Framework"]
        E1[Heterogeneous Router] --> E2[PostgreSQL / SQLite / MongoDB / DuckDB]
        E4[Diagnostic Engine] -- Detects Failures --> E5[Autonomous Recovery]
        E5 -- Re-runs Query --> E1
        E1 --> E6[Execution Sandbox: Local or Cloudflare]
    end

    subgraph Infrastructure ["Shared Infrastructure: tenai-infra"]
        F1[Tailscale Mesh Network] --> F2[Shared Mob Sessions]
        F3[MCP Toolbox for Databases] --> E1
    end

    subgraph Evaluation ["Evaluation Science: The Sentinel"]
        G1[Trace & Score Harness] --> G2[DAB Benchmark: 54 Queries]
        G1 --> G3[Adversarial Probe Library]
    end

    %% Connections
    Governance -. Guides .-> Execution
    Context -- Grounds Reasoning --> Execution
    Execution -- Logs Results --> Context
    Execution -- Tool Traces --> Evaluation
    Infrastructure -- Hosts --> Execution
```
---
